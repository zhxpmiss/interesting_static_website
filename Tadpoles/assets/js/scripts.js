...
function isVisible(el) {
    ...
    return top > viewTop && bottom <= viewBottom || top <= viewTop && bottom > viewTop || top <= viewBottom && bottom > viewBottom;
}
...
function createPaperScript(element) {
    ...
    function resize() {
        source.width(element.width() - (hasBorders ? 2 : 1)).height(element.height() - (hasBorders ? 2 : 0));
        if (editor) {
            editor.refresh();
        }
    }
